...
class WalletTransactionsViewTests(APITestCase):
    ...
    def test_filter_by_amount_range(self):
        ...
        self.client.force_authenticate(user=self.user)
        response = self.client.get(self.url, {'min_amount': '400', 'max_amount': '600'})

        self.assertEqual(response.status_code, status.HTTP_200_OK)
        for transaction in response.data['results']:
            if transaction['amount']:
                amount = Decimal(transaction['amount'])
                self.assertGreaterEqual(amount, Decimal('400'))
                self.assertLessEqual(amount, Decimal('600'))

    def test_search_transactions(self):
        """Test searching transactions by description or reference"""
        transaction = WalletTransaction.objects.create(
            ...
            payment_method='bank',
            status='success',
            description='Test deposit for search'
        )
    ...
    def test_invalid_filter_parameters(self):
        """Test that invalid filter parameters return validation error"""
        self.client.force_authenticate(user=self.user)
        response = self.client.get(self.url, {'min_amount': '1000', 'max_amount': '100'})
